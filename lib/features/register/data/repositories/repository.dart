...
class RegisterRepoImpel implements RegisterRepo {
  ...
  Future<Either<Failure, int>> sendOtp({required String phone}) async {
    try {
      Response res = await dio.postData(url: '/register/send-otp', data: {'mobile': phone});
  ...
  @override
  Future<Either<Failure, int>> verifyCode({required int code, required String phoneNum, required String type}) async {
    try {
      Response res = await dio.postData(url: '/register/verify-code', data: {'mobile': phoneNum, "code": code, 'type': type});
      return   Right(res.data['user_id']);
    }on DioError catch (e) {
      debugPrint(e.response.toString());
      return Left(DioResponseFailure(errorModel: ErrorModel.fromMap(e.response!.data! as Map<String,dynamic>  )));
    }
  ...
  Future<Either<Failure, String>> sendCompleteProfileData( {required UserModel userModel,required XFile xFile}) async{
    try {
      ...
      Response res = await dio.postDataWithFile(url: '/register/user-data', data:   userModel.toMap(userModel: userModel) , xFile: xFile, name: 'avatar');
